...
int16_t test_runningshoot_speed = -5500;
...
void shoot_work(shoot_t* shoot)
{
	...
	/*视觉射速处理*/
	L_shoot_speed     = -((vision_structure.rx_pack->RxData.left_aim_data.shoot_frequency/10.0)/8.0)*60*36;
	L_shoot_cnt_last  =    L_shoot_cnt;
	L_shoot_cnt       =    vision_structure.rx_pack->RxData.left_aim_data.shoot_value;
	
	R_shoot_speed     = -((vision_structure.rx_pack->RxData.right_aim_data.shoot_frequency/10.0)/8.0)*60*36;
	R_shoot_cnt_last  =    R_shoot_cnt;
	R_shoot_cnt       =    vision_structure.rx_pack->RxData.right_aim_data.shoot_value;
	...
	/*枪管热量限制*/
	/*双枪未连服务器只有一根枪管热量*/
	 judge.data->power_heat_data.shooter_id1_17mm_cooling_heat = 240;
	 judge.data->power_heat_data.shooter_id2_17mm_cooling_heat = 240;
	
	if(judge.base_info->shooter_id2_cooling_heat >= judge.data->game_robot_status.shooter_barrel_heat_limit - 80 )
	{
		if(L_shoot_speed <= -2400)
		{
			L_shoot_speed = -2400;
		}
		if(judge.base_info->shooter_id2_cooling_heat >= judge.data->game_robot_status.shooter_barrel_heat_limit - 45 )
		{
            //测试注，记得取
			L_shoot_speed = 0;
			L_shoot_structure.stir_wheel->base_info->target_angle_sum = 0;
	...
	if(judge.base_info->shooter_id1_cooling_heat >= judge.data->game_robot_status.shooter_barrel_heat_limit - 80 )
	{
		if(R_shoot_speed <= -2400)
		{
			R_shoot_speed = -2400;
		}
		if(judge.base_info->shooter_id1_cooling_heat >= judge.data->game_robot_status.shooter_barrel_heat_limit - 45 )
		{
			R_shoot_speed = 0;
			R_shoot_structure.stir_wheel->base_info->target_angle_sum = 0;
			R_shoot_structure.stir_wheel->base_info->angle_sum = 0;
	...
	if(L_shoot_structure.status == Running_Shoot)
	{
		...
		{
			if(judge.base_info->shooter_id2_cooling_heat >= judge.data->power_heat_data.shooter_id2_17mm_cooling_heat - 50)
            {
			...
			// {
			// 	L_shoot_structure.base.shoot_speed = test_runningshoot_speed;//-5500;
			// }
			// else
			// {
			// 	L_shoot_structure.base.shoot_speed = test_runningshoot_speed;
            // }
            //测试用，记得注，左右头各一个
	...
	if(R_shoot_structure.status == Running_Shoot)
	{
		...
		{
			if(judge.base_info->shooter_id1_cooling_heat >= judge.data->power_heat_data.shooter_id1_17mm_cooling_heat - 50)
			{
            ...
            // 测试用，记得注，左右头各一个
			// {
			// 	R_shoot_structure.base.shoot_speed = test_runningshoot_speed;//-5500;
			// }
			// else
			// {
			// 	R_shoot_structure.base.shoot_speed = test_runningshoot_speed;
			// }
