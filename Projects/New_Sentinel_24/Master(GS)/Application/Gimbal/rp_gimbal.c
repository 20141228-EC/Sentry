...
void Pitch_Auto_R(Master_Head_t* M2H,uint16_t up_most)
{
	...
	if(M2H->Send_R_Head.target_pit <= P_MOT_LOW_LIMIT_R + up_most)
	{
		direction_p_R = Auto_Down;
	}
	else if(M2H->Send_R_Head.target_pit >= P_MOT_UPP_LIMIT_R)
	{
...
void Pitch_Auto_L(Master_Head_t* M2H,uint16_t up_most)
{
	...
	if(M2H->Send_L_Head.target_pit >= P_MOT_UPP_LIMIT_L - up_most)
	{
...
{
	static uint8_t direction_L = Auto_Left;  //初始巡航方向
	//static uint8_t cnt = 0;        //改变目标值的时间间隔
	
	if(M2H->Send_L_Head.target_yaw >= Y_MOT_LOW_LIMIT_L &&  M2H->Send_L_Head.target_yaw<= Y_MOT_LOW_LIMIT_L + 100)
	{
		direction_L = Auto_Left;
	}
	else if(M2H->Send_L_Head.target_yaw <= Y_MOT_UPP_LIMIT_L && M2H->Send_L_Head.target_yaw>= Y_MOT_UPP_LIMIT_L - 100)
	{
...
void Yaw_Auto_R(Master_Head_t* M2H)
{
	...
	if( M2H->Send_R_Head.target_yaw >= Y_MOT_LOW_LIMIT_R && M2H->Send_R_Head.target_yaw <= Y_MOT_LOW_LIMIT_R + 100)
	{
		direction_R = Auto_Left;
	}
	else if(M2H->Send_R_Head.target_yaw <= Y_MOT_UPP_LIMIT_R && M2H->Send_R_Head.target_yaw >= Y_MOT_UPP_LIMIT_R - 100)
	{
