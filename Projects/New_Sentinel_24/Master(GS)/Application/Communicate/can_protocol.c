 ...
 *	@brief	底盘掉线检测
 */
void Master_Chassis_HEART(Master_Head_t * pack)
{
	if(pack->Chassis_status.offline_cnt ++ >= M2H_OFFLINE_TIME_MAX)
	{
		pack->Chassis_status.status = M2H_OFFLINE;
		pack->Chassis_status.offline_cnt = M2H_OFFLINE_TIME_MAX;
	}
	else
	{
		pack->Chassis_status.status = M2H_ONLINE;
	}
}
...
void Master_Head_INIT(Master_Head_t * pack)
{
	...
	pack->Send_L_Head.target_yaw = Y_Init_L;
	pack->Send_L_Head.target_pit = P_Init_L;
	pack->Send_L_Head.target_pit = M2H_MODE_INIT;
	
	pack->Send_R_Head.target_yaw = Y_Init_R;
	pack->Send_R_Head.target_pit = P_Init_R;
	pack->Send_R_Head.target_pit = M2H_MODE_INIT;

	pack->Send_Chassis.speed_front = 0;
	pack->Send_Chassis.speed_right = 0;
	pack->Send_Chassis.speed_cycle = 0;
	pack->Send_Chassis.buffer = -1;		//默认关控卸力
 ...
 * @brief	数据信息发送（至底盘）
*/
void M2C_SENT_CHASSIS(Master_Head_t* pack)
{
	...
	/*数据处理*/
	data[0] = (uint8_t)((int16_t)pack->Send_Chassis.speed_front >> 8);
	data[1] = (uint8_t)((int16_t)pack->Send_Chassis.speed_front);
	
	data[2] = (uint8_t)((int16_t)pack->Send_Chassis.speed_right >> 8);
	data[3] = (uint8_t)((int16_t)pack->Send_Chassis.speed_right);
	
	data[4] = (uint8_t)((int16_t)pack->Send_Chassis.speed_cycle >> 8);
	data[5] = (uint8_t)((int16_t)pack->Send_Chassis.speed_cycle);
	
	data[6] = (uint8_t)((int16_t)pack->Send_Chassis.buffer >> 8);
	data[7] = (uint8_t)((int16_t)pack->Send_Chassis.buffer);		
	/*数据帧开头*/
	txFrameHeader.StdId = SEND_CHASSIS_ID;
	txFrameHeader.IDE   = CAN_ID_STD;
	txFrameHeader.RTR   = CAN_RTR_DATA;
	txFrameHeader.DLC   = 0x08;

	/*Master CAN1 to Down CAN2*/
	/*CAN1发送*/
	HAL_CAN_AddTxMessage(&M2C_MASTER_CAN_USE, &txFrameHeader, data, &txMailBox);
 ...
 * @brief	底盘信息反馈
*/
void M2C_BACK_CHASSIS(Master_Head_t* pack)
{
	uint32_t txMailBox;//发送邮箱
	CAN_TxHeaderTypeDef txFrameHeader;
	uint8_t data[8];
	...
	/*数据处理*/
	data[0] = (uint8_t)((int16_t)pack->From_Chassis.mode1 >> 8);
	data[1] = (uint8_t)((int16_t)pack->From_Chassis.mode1);

	data[2] = (uint8_t)((int16_t)pack->From_Chassis.mode2 >> 8);
	data[3] = (uint8_t)((int16_t)pack->From_Chassis.mode2);

	data[4] = (uint8_t)((int16_t)pack->From_Chassis.mode3 >> 8);
	data[5] = (uint8_t)((int16_t)pack->From_Chassis.mode3);

	data[6] = (uint8_t)((int16_t)pack->From_Chassis.mode4 >> 8);
	data[7] = (uint8_t)((int16_t)pack->From_Chassis.mode4);
		
	/*数据帧开头*/
	txFrameHeader.StdId = FROM_CHASSIS_ID;
	txFrameHeader.IDE   = CAN_ID_STD;
	txFrameHeader.RTR   = CAN_RTR_DATA;
	txFrameHeader.DLC   = 0x08;

	/*Down CAN2 to Master CAN1*/
	/*CAN2发送*/
	HAL_CAN_AddTxMessage(&M2C_DOWN_CAN_USE, &txFrameHeader, data, &txMailBox);
...
void H2M_GETT_DATA( uint32_t id ,Master_Head_t * pack , uint8_t *rxBuf)
{
	switch (id)
	{
		...
		case SEND_CHASSIS_ID:
			pack->Send_Chassis.speed_front = (int16_t)rxBuf[0];
			pack->Send_Chassis.speed_front <<= 8;
			pack->Send_Chassis.speed_front |= (int16_t)rxBuf[1];
		
			pack->Send_Chassis.speed_right = (int16_t)rxBuf[2];
			pack->Send_Chassis.speed_right <<= 8;
			pack->Send_Chassis.speed_right |= (int16_t)rxBuf[3];
		
			pack->Send_Chassis.speed_cycle = (int16_t)rxBuf[4];
			pack->Send_Chassis.speed_cycle <<= 8;
			pack->Send_Chassis.speed_cycle |= (int16_t)rxBuf[5];
		
			pack->Send_Chassis.buffer = (int16_t)rxBuf[6];
			pack->Send_Chassis.buffer <<= 8;
			pack->Send_Chassis.buffer |= (int16_t)rxBuf[7];
			break;
			
		case FROM_CHASSIS_ID:
			pack->From_Chassis.mode1 = (int16_t)rxBuf[0];
			pack->From_Chassis.mode1 <<= 8;
			pack->From_Chassis.mode1 |= (int16_t)rxBuf[1];
		
			pack->From_Chassis.mode2 = (int16_t)rxBuf[2];
			pack->From_Chassis.mode2 <<= 8;
			pack->From_Chassis.mode2 |= (int16_t)rxBuf[3];
		
			pack->From_Chassis.mode3 = (int16_t)rxBuf[4];
			pack->From_Chassis.mode3 <<= 8;
			pack->From_Chassis.mode3 |= (int16_t)rxBuf[5];
		
			pack->From_Chassis.mode4 = (int16_t)rxBuf[6];
			pack->From_Chassis.mode4 <<= 8;
			pack->From_Chassis.mode4 |= (int16_t)rxBuf[7];
			
			pack->Chassis_status.offline_cnt = 0;//掉线清零
			break;
	}
