...
/*底盘定义*/
#define SEND_CHASSIS_ID 0x001
#define FROM_CHASSIS_ID 0x002

#define MH_DRV_CAN_USE     		(hcan2)  /*hcan1  hcan2*/
#define M2C_DOWN_CAN_USE     	(hcan2)
#define M2C_MASTER_CAN_USE     	(hcan1)
...
typedef struct __packed 
{
	...
	int16_t speed_right;
	int16_t speed_cycle;
	int16_t buffer;     	// 若buffer小于0 则代表关控卸力   其实这项数据可以直接听超电的 
