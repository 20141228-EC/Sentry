...
#define LEFT_YAW_MID_ANGLE  Y_Init_L
#define LEFT_PITCH_MID_ANGLE P_Init_L

#define RIGHT_YAW_MID_ANGLE Y_Init_R
#define RIGHT_PITCH_MID_ANGLE P_Init_R
...
//				vision_structure.rx_pack->RxData.L_shoot_cnt += 3;
                ...
                //leftyaw_test = vision_rx_pack.RxData.left_aim_data.target_yaw;
				...
				// vision_rx_pack.RxData.left_aim_data.flag.bit.is_find_target = 
				...
				/*没干死广工的秘密武器
				if(vision_rx_pack.RxData.right_aim_data.target_pitch <= P_MOT_LOW_LIMIT_R || vision_rx_pack.RxData.right_aim_data.target_pitch >= 4000)
				{
					vision_rx_pack.RxData.right_aim_data.target_pitch = P_MOT_LOW_LIMIT_R;
				}
				else if(vision_rx_pack.RxData.right_aim_data.target_pitch >= P_MOT_UPP_LIMIT_R && vision_rx_pack.RxData.right_aim_data.target_pitch <= 4000)
				{
					vision_rx_pack.RxData.right_aim_data.target_pitch = P_MOT_UPP_LIMIT_R;
				}

				if(vision_rx_pack.RxData.left_aim_data.target_pitch <= P_MOT_LOW_LIMIT_R_ && vision_rx_pack.RxData.left_aim_data.target_pitch >= 3000)
				{
					vision_rx_pack.RxData.left_aim_data.target_pitch = P_MOT_LOW_LIMIT_R_;
				}
				else if(vision_rx_pack.RxData.left_aim_data.target_pitch >= P_MOT_UPP_LIMIT_R_ || vision_rx_pack.RxData.left_aim_data.target_pitch <= 3000)
				{
					vision_rx_pack.RxData.left_aim_data.target_pitch = P_MOT_UPP_LIMIT_R_;
				}*/
...
{	
	...
	/*发送数据处理*/
		...
		/* 击打模式 0-默认 1-自瞄 2/3/5打符 4-初始化 6-哨兵 7-前哨 8-基地 9-英雄 10-工程 */
		...
		/* 比赛信息 */
		vision_structure.tx_pack->TxData.own_remain_bullets = judge.base_info->remain_bullet;
		vision_structure.tx_pack->TxData.shoot_speed = 30;	//射速固定30

		/* 2024.5 新增比赛时间，当前位置*/
		// vision_structure.tx_pack->TxData.game_time 写在裁判心跳函数
		vision_structure.tx_pack->TxData.uwb_position_x = judge.base_info->uwb_position_x;
		vision_structure.tx_pack->TxData.uwb_position_y = judge.base_info->uwb_position_y;
		...
		vision_structure.tx_pack->TxData.armor5_size = 0;*/		//赋值放在task内的函数
		
		/* 是否击打目标 */
		/* 0-打 1-不打 */
		vision_structure.tx_pack->TxData.no_hit_enemy2 = navigation_structure.rx_pack->RxData.no_hit_enemy2;			//不打工程
		vision_structure.tx_pack->TxData.no_hit_enemy6 = navigation_structure.rx_pack->RxData.no_hit_enemy6;			//不打哨兵
		vision_structure.tx_pack->TxData.no_hit_enemy7 = navigation_structure.rx_pack->RxData.no_hit_enemy7;			//不打前哨
        vision_structure.tx_pack->TxData.no_hit_enemy8 = navigation_structure.rx_pack->RxData.no_hit_enemy8;			//不打基地
		
		
        /* 血量 6哨兵 7前哨站 8基地 */
        /* 为了保证在uint8_t的范围 ：blood0-6（机器人） 是 实时血量÷5； blood7-8（建筑） 是 实时血量÷10 */
		/* 己方 */
		vision_structure.tx_pack->TxData.own_blood6   = (uint8_t)((judge.base_info->remain_HP + 4) / 5);
		vision_structure.tx_pack->TxData.own_blood7   = (uint8_t)((judge.base_info->ally_outpost_HP + 4) / 5);
		vision_structure.tx_pack->TxData.own_blood8   = (uint8_t)((judge.base_info->ally_base_HP + 4) / 5);
		
		/* 敌方 */
		vision_structure.tx_pack->TxData.enemy_blood1 = (uint8_t)((judge.base_info->enemy_hero_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood2 = (uint8_t)((judge.base_info->enemy_engineer_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood3 = (uint8_t)((judge.base_info->enemy_infantry3_HP + 4) / 5);
        vision_structure.tx_pack->TxData.enemy_blood4 = (uint8_t)((judge.base_info->enemy_infantry4_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood5 = (uint8_t)((judge.base_info->enemy_infantry5_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood6 = (uint8_t)((judge.base_info->enmey_sentry_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood7 = (uint8_t)((judge.base_info->enemy_outpost_HP + 4) / 5);
		vision_structure.tx_pack->TxData.enemy_blood8 = (uint8_t)((judge.base_info->enemy_base_HP + 4) / 5);
		
		/* 全向感知，来自决策端 */
		memcpy(&vision_structure.tx_pack->TxData.omni_enemy_data, &navigation_structure.rx_pack->RxData.omni_enemy_data, sizeof(EnemyData_t));
