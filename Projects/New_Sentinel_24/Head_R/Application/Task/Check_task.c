...
void CheckTask(void const * argument)
{
	...
	{	
		...
		if(Master_Head_structure.R_Head_status.status == M2H_OFFLINE)
		{
			LASER_OFF();
		}
		else
...
void H2MTask(void const * argument)
{
	while(1)
	{
		...
		gimbal_structure.pitch->base_info->target_angle = Master_Head_structure.Send_R_Head.target_pit;
		gimbal_structure.yaw->base_info->target_angle   = Master_Head_structure.Send_R_Head.target_yaw;
		...
		// H2M_SENT_DATA(&Master_Head_structure);
