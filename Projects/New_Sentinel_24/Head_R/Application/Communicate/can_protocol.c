 ...
 * @brief	ljx
*/
void H2M_SENT_DATA(Master_Head_t * pack)
{
	uint32_t txMailBox;//发送邮箱
	CAN_TxHeaderTypeDef txFrameHeader;
	uint8_t data[8];
	...
	data[2] = (uint8_t)((int16_t)pack->From_R_Head.measure_yaw >> 8);
	data[3] = (uint8_t)((int16_t)pack->From_R_Head.measure_yaw);
	
	data[4] = (uint8_t)((int16_t)pack->From_R_Head.measure_pit >> 8);
	data[5] = (uint8_t)((int16_t)pack->From_R_Head.measure_pit);
	
	data[6] = (uint8_t)((int16_t)pack->From_R_Head.gimbal_mode);
	data[7] = (uint8_t)((int16_t)pack->From_R_Head.shoot_mode);
	
	
	/*数据帧开头*/
	txFrameHeader.StdId = FROM_R_HEAD_ID;
	txFrameHeader.IDE   = CAN_ID_STD;
	txFrameHeader.RTR   = CAN_RTR_DATA;
	txFrameHeader.DLC   = 0x08;//?
