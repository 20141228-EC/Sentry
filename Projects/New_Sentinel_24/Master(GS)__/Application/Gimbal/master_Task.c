...
void master_work(void)
{
	...
	if(gimbal_init_ok == 0)
	{
		...
		Master_Head_structure.Send_R_Head.target_pit = P_Init_L;
		Master_Head_structure.Send_R_Head.target_yaw = Y_Init_L;
		gimbal_init_ok = 1;
	}
...
//			}
	...
	{
		Master_Head_structure.Send_L_Head.target_pit  = P_Init_L - error;
		Master_Head_structure.Send_L_Head.target_yaw  = Y_Init_L;
		
		Master_Head_structure.Send_R_Head.target_pit  = P_Init/* - error机械模式测散布*/;
		Master_Head_structure.Send_R_Head.target_yaw  = Y_Init;
...
/* 	if(vision_structure.tx_pack->TxData.flag.bit.only_outpose == 0)
	...
	{
		R_Dynamic_Pit_High = 622;
	}
	else if (Master_Head_structure.From_R_Head.measure_yaw >= 7080)
	{
		R_Dynamic_Pit_High = 690;
	}
	else
	{
		R_Dynamic_Pit_High = 432;//所能最高
	}
	//头最低，值最大
	...
	{
		R_Dynamic_Pit = 1811;//头最低，值最大
	}
	else
	{
		R_Dynamic_Pit = 2040;//所能最低
	}
	...
	/*L全局动态限位*/
	//头高，值大*左右头电机反置
	if(Master_Head_structure.From_L_Head.measure_yaw <= Y_MOT_LOW_LIMIT - 50 && Master_Head_structure.From_L_Head.measure_yaw >= Y_MOT_MID_LIMIT)
	{
		L_Dynamic_Pit_High = 4747;//躲雷达
	}
	...
	{
		L_Dynamic_Pit_High = 4725;//躲灯条
	}
	else
	{
		L_Dynamic_Pit_High = 4930;//所能最高 mechMAX
	}
	//头最低，值最小
	if(Master_Head_structure.From_L_Head.measure_yaw <= 5271)
	{
		L_Dynamic_Pit = 3823;//后平台板
	}
	else if(Master_Head_structure.From_L_Head.measure_yaw <= 2227)
	{
		L_Dynamic_Pit = 3850;//前平台板
	}
	else
	{
		L_Dynamic_Pit = 3410;//所能最低 mechMIN
	}
