...
void Yaw_Auto_R(Master_Head_t* M2H)
{
	...
	if( M2H->Send_R_Head.target_yaw >= Y_MOT_LOW_LIMIT && M2H->Send_R_Head.target_yaw <= Y_MOT_LOW_LIMIT + 100)
	{
		direction_R = Auto_Left;
	}
	else if(M2H->Send_R_Head.target_yaw <= Y_MOT_UPP_LIMIT && M2H->Send_R_Head.target_yaw >= Y_MOT_UPP_LIMIT - 100)
	{
