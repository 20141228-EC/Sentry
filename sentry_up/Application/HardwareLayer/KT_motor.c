...
int16_t Lowpass_kt(int16_t X_last, int16_t X_new, float K)
{
...
void KT_Motor_ToAngle(kt_motor_t *motor, uint32_t angle)
{
    ...
    motor->motor_pid->pos_speed.measure = Lowpass_kt(last_speed,motor->info->rx_info->speed,0.7);
	last_speed = motor->info->rx_info->speed;
	motor->motor_pid->pos_speed.err = motor->motor_pid->pos_angle.out - motor->info->rx_info->speed;
	//速度环
	kt_single_pid_ctrl(&motor->motor_pid->pos_speed);
	//发送数据到数组
    if(motor->work_state == DEV_ONLINE)
    {
