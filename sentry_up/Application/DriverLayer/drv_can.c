...
uint8_t CHASSIS_DATA[8] = {0};
uint8_t FLAG_DATA[8] = {0};
...
CAN_RxFrameTypeDef hcan1RxFrame;		//接收header，接收数据
...
/**
  ...
  * @retval 
  */
void CAN1_CMD_200()		//控制1~4电调，发送数据为CAN_200_Data
{
...
void CAN2_CMD_200()
{
	...
	state = HAL_CAN_AddTxMessage(&hcan2, &tx_message, CAN2_200_DATA, &send_mail_box);
}
...
void CAN2_CMD_CHS()
{
	...
	uint32_t send_mail_box;
	tx_message.StdId = 0x001;
	tx_message.IDE = CAN_ID_STD;
	tx_message.RTR = CAN_RTR_DATA;
	tx_message.DLC = 0x08;

	HAL_CAN_AddTxMessage(&hcan2, &tx_message, CHASSIS_DATA, &send_mail_box);
}
...
void CAN2_CMD_FLAG()
{
	...
	uint32_t send_mail_box;
	tx_message.StdId = 0x003;
	tx_message.IDE = CAN_ID_STD;
	tx_message.RTR = CAN_RTR_DATA;
	tx_message.DLC = 0x08;

	HAL_CAN_AddTxMessage(&hcan2, &tx_message, FLAG_DATA, &send_mail_box);
}
...
/**
  ...
  * @retval None
  */
void int16_to_uint8(uint8_t *data, int16_t *dat)		//好像只在cap里有用到
{
...
/**
  ...
  * @retval 
  */
void CAN_Filter_Init(void)	//这本来也是can.h中的
{
	//在can.h中配置好，然后在这里调用
	HAL_CAN_Init(&hcan1);
	HAL_CAN_Init(&hcan2);
