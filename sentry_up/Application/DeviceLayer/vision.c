...
void vision_heartbeat(Vision_t *vis_sen)
{
	...
	if(vis_info->offline_cnt > vis_info->offline_max_cnt)
	{
		...
		vis_sen->info->is_find_target = 0;
		vis_sen->info->fire_cnt = 0;
	}
	else
	{
		/* 离线->在线 */
...
{
	memcpy(Vision_txBuf, vision.info->tx_info, sizeof(Vision_tx_info_t));

	Append_CRC8_Check_Sum(Vision_txBuf, 3);
	vision.info->tx_info->CRC8 = Vision_txBuf[2];
	Append_CRC16_Check_Sum(Vision_txBuf, sizeof(Vision_tx_info_t));
	vision.info->tx_info->CRC16 = (uint16_t)((Vision_txBuf[sizeof(Vision_tx_info_t)-2] << 8)
                                  | Vision_txBuf[sizeof(Vision_tx_info_t)-1]);

	if(CDC_Transmit_FS(Vision_txBuf,sizeof(Vision_tx_info_t)) == USBD_OK)
	{
			return true;
	}
...
void vision_check(Vision_t *vis_sen)
{
	...
	if (info->rx_flag == 1)		// 接收成功
	{
		...
		memcpy(&info->fire_cnt, &rx_info->fire_cnt, 1);
        
		// 云台角度转换
		info->target_pitch_angle = -(info->target_pitch_angle * ECD_TO_ANGLE - 180.0f);		//已测试24.12.9
    	info->target_yaw_angle = (info->target_yaw_angle * ECD_TO_ANGLE - 180.0f);
	}
}
/*-------- protocol 函数 end --------*/
