...
cap_t cap = {
	...
	.info.tx.output_power_limit = 280,
...
//    self->info.tx.cap_control.bit.cap_switch = Chassis_move.cap_switch;
    self->info.tx.cap_control.bit.cap_record = 0;   //这里不确定含义,待改!!!
...
///电容接受int16_t的数据( 32000, -32000),转化成实际Ucr为(30,0)的浮点数
...
{
    if(self->info.rx.cap_Ucr > 23)
    {
        self->info.tx.input_power_limit = 30;
    }
//    else if(self->info.rx.cap_state.bit.cap_I_over == 1 || self->info.rx.cap_state.bit.cap_U_over == 1)
//    {
//        self->info.tx.input_power_limit = 0;
//    }
    else
    {
        self->info.tx.input_power_limit = 150;
    }
}
