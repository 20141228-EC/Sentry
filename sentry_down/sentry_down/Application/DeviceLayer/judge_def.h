 ...
 * @update		修订命令码 0x0101、0x0201、0x0203、0x0209、0x020A、0x0303
				...
				修改底盘功率和枪口热量数据发送频率为50Hz(原为10Hz)
				修改飞镖机器人客户端指令发送频率为3Hz(原为10Hz)
				修改机器人交互数据，发送频率上限为30Hz(原为10Hz)
				修改选手端小地图接收雷达数据，频率上限为5Hz(原为10Hz)
				修改选手端小地图接收哨兵数据，频率上限为1Hz(原为3Hz)

				修改LEN_GAME_ROBOT_POS为12(原为16)
				修改LED_SENTRY_AUTO_INFO为6(原为4)

				101场地事件数据可能需要更改，待更新
				102补给站动作标识数据可能需要更改，待更新
				209RFID能量机关激活点可能需要更改，待更新
...
{ 
	...
	ID_power_heat_data    					= 0x0202,	// 实时底盘功率和枪口热量数据，50Hz
	...
	ID_robot_interactive_header_data					= 0x0301,	// 机器人交互数据，发送方触发发送，频率上限为 30Hz
	ID_controller_interactive_header_data 		= 0x0302,	// 自定义控制器与机器人交互数据，发送方触发发送，频率上限为 30Hz	/*图传链路*/
	ID_map_interactive_header_data        		= 0x0303,	// 选手端小地图交互数据，选手端触发发送
	ID_keyboard_information               		= 0x0304,	// 键键鼠遥控数据，30Hz											   /*图传链路*/
	ID_smallmap                           		= 0x0305,	// 选手端小地图接收雷达数据，频率上限为 5Hz
	ID_custom_client_data                 		= 0x0306,	// 自定义控制器与选手端交互数据，发送方触发发送，30Hz
	ID_map_sentry_data                    		= 0x0307,	// 选手端小地图接收哨兵数据， 仅云台手客户端接收，1Hz
...
typedef __packed struct 
{ 
	uint32_t unuse:4;		// 占领情况
	uint32_t finish_small_buff_state:1;	//小符激活状态
	uint32_t unuse_:27;
								// 己方前哨战状态
								// 环形高地占领状态
								// 己方基地护盾状态
...
typedef __packed struct 
{
	...
	uint32_t if_revive_await				:1;		//bit0 		哨兵机器人是否确认复活 
	uint32_t if_revive_immediate		:1;		//bit1 		哨兵机器人是否确认兑换立即复活 
	uint32_t add_bullet_num					:11;	//bit2-12	哨兵将要兑换的发弹量值
	uint32_t remote_add_bullet_times:4;		//bit13-16哨兵远程兑换发弹量的请求次数
	uint32_t remote_add_HP_times		:4;		//bit17-20哨兵远程兑换血量的请求次数
	uint32_t unuse_bits							:11;	//bit21-31保留
}sentry_cmd_t;

/* ID：0x020D	Byte：6		哨兵自主决策信息同步（rx）*/
typedef __packed struct 
{
