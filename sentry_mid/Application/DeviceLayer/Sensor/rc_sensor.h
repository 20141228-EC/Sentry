...
#define    RC_TB_VALUE              (rc_sensor_info.thumbwheel.value)
#define    RC_TB_MID2UP             (rc_sensor_info.thumbwheel.MID2UP)    //从中到上一次为0，1跳变
#define    RC_TB_MID2DOWN           (rc_sensor_info.thumbwheel.MID2DOWN)
...
/* 检测按键长按时间 */
#define MOUSE_BTN_L_CNT_MAX     350         //ms 鼠标左键
...
/* ----------------------- Function Definition-------------------------------- */
/* 拨杆当前值 */
#define		RC_SW1_VALUE				  (rc_sensor_info.s1.value)
#define		RC_SW2_VALUE				  (rc_sensor_info.s2.value)
#define   RC_SW1_VALUE_LAST			(rc_sensor_info.s1.value_last)
#define   RC_SW2_VALUE_LAST			(rc_sensor_info.s2.value_last)

/* 遥控拨杆跳变 */
#define   RC_SW1_STEP         (rc_sensor_info.s1.value != rc_sensor_info.s1.value_last)
#define   RC_SW2_STEP         (rc_sensor_info.s2.value != rc_sensor_info.s2.value_last)

#define   RC_SW1_UP2MID       (rc_sensor_info.s1.value == RC_SW_MID && rc_sensor_info.s1.value_last == RC_SW_UP)
#define   RC_SW1_MID2UP       (rc_sensor_info.s1.value == RC_SW_UP && rc_sensor_info.s1.value_last == RC_SW_MID)
#define   RC_SW1_MID2DOWN     (rc_sensor_info.s1.value == RC_SW_DOWN && rc_sensor_info.s1.value_last == RC_SW_MID)
#define   RC_SW1_DOWN2MID     (rc_sensor_info.s1.value == RC_SW_MID && rc_sensor_info.s1.value_last == RC_SW_DOWN)

#define   RC_SW2_UP2MID       (rc_sensor_info.s2.value == RC_SW_MID && rc_sensor_info.s2.value_last == RC_SW_UP)
#define   RC_SW2_MID2UP       (rc_sensor_info.s2.value == RC_SW_UP && rc_sensor_info.s2.value_last == RC_SW_MID)
#define   RC_SW2_MID2DOWN     (rc_sensor_info.s2.value == RC_SW_DOWN && rc_sensor_info.s2.value_last == RC_SW_MID)
#define   RC_SW2_DOWN2MID     (rc_sensor_info.s2.value == RC_SW_MID && rc_sensor_info.s2.value_last == RC_SW_DOWN)
...
/* 遥控器通道值 */
#define		RC_LEFT_CH_LR_VALUE			(rc_sensor_info.ch2)  //左通道Left Right
#define		RC_LEFT_CH_UD_VALUE			(rc_sensor_info.ch3)  //左通道Up Down
...
/* 检测鼠标按键状态 按下为1，没按下为0*/
#define    MOUSE_PRESSED_LEFT    (rc_sensor_info.mouse_btn_l.value)
#define    MOUSE_PRESSED_RIGH    (rc_sensor_info.mouse_btn_r.value)

/* 检测鼠标短按长按 */
#define    MOUSE_LEFT_SHORT_PRESS    (rc_sensor_info.mouse_btn_l.last_status == short_press && rc_sensor_info.mouse_btn_l.status == press_to_release)
#define    MOUSE_LEFT_LONG_PRESS     (rc_sensor_info.mouse_btn_l.status == long_press)
...
typedef struct rc_sensor_struct {
	rc_sensor_info_t	*info;				//存储信息
	drv_uart_t		  	*driver;			//驱动
	void				(*init)(struct rc_sensor_struct *self);											//初始化函数			
	void				(*update)(struct rc_sensor_struct *self, uint8_t *rxBuf);		//更新信息函数
	void				(*check)(struct rc_sensor_struct *self);										//检查？函数
	void				(*heart_beat)(struct rc_sensor_struct *self);								//失联检测函数
...
void rc_sensor_s_step(rc_sensor_t *rc_sen);
void rc_sensor_thumbwheel_step(rc_sensor_t *rc_sen);
