 ...
 * @LastEditTime: 2025-07-26 18:44:40
 * @FilePath: \sentry_mid\Application\HardwareLayer\DM_motor.h
 * @Description: 
 * 
 */
#ifndef __DM_MOTOR_H
...
typedef struct dm_motor_info_struct
{
    ...
    uint16_t offline_max_cnt; // 离线最大�?�数
...
{	
    uint16_t CAN_Rx_ID;
    // 发数�?绑定的CAN ID, �?上位机驱动参数CAN_ID加上控制模式的偏移量
    uint16_t CAN_Tx_ID;
...
{
	pid_ctrl_t mech_angle_pid; // 双环机�?��?�度PID
	pid_ctrl_t mech_speed_pid; // 双环机�?�速度PID
...
typedef struct dm_motor_struct
{
    ...
    dev_work_state_t work_state; // 工作状�?
